...
function Ivgenerator(){//Randpm IV Generator
...
//for CBC Mode
$("#p_t1").keydown(function () {//when you click on plaintext box 
    $("#c_t1").val("");//ciphertext block becomes empty
});
$("#c_t1").keydown(function () {//when you click on ciphertext block
    $("#p_t1").val("");//plaintext block becomes empty
});
$("#encrypt1").click(function () {//on clicking encrypt button
    var Plain_text = $("#p_t1").val();// var Plain_text stores the entered plaintext in hexadecimal format
    var key=$("#key1").val();// var key stores the key entered in hexadecimal format
    var Iv=obj.IV;//Iv stores the initial value generated by random generator
    var Cipher_text="";//will contain final ciphertext
    for(var i=0;i<Plain_text.length;i+=16){
        Ptext_block=obj.xor(Plain_text.substr(i,16).padStart(16,'0'),Iv); /*Ptext_block that goes for encryption is XOR of 64 bits of Ptext 
        ...
        Cipher_text+=Ctext_block;
        Iv=Ctext_block;
    }
    $("#c_t1").val(Cipher_text);//store value of end Ciphertext in c_t1 element
});
$("#decrypt1").click(function () {//on clicking decrypt button
    var Cipher_text = $("#c_t1").val();//ciphertext stores the entered ciphertext
    var key=$("#key1").val();//enter the same key (it will be used in reverse order)
    var Iv=obj.IV;//Iv stores the initial value generated by random generator
    var Plain_text="";//will store the final plaintext
    for(var i=0;i<Cipher_text.length;i+=16){
        var Ptext_block=obj.Decrypt(Cipher_text.substr(i,16).padStart(16,'0'),key);//Decrypt the ciphertext 
        Ptext_block=obj.xor(Ptext_block,Iv);// Ptext_block xored with Iv
        Plain_text+=Ptext_block;
        Iv=Cipher_text.substr(i,16);
    }
    $("#p_t1").val(Plain_text);//store value of end Plaintext in p_t1 element
});

// for OFB Mode
$("#p_t2").keydown(function () {//when you click on plaintext block
    $("#c_t2").val("");//ciphertext block becomes empty
});
$("#c_t2").keydown(function () {//when you click on ciphertext block
    $("#p_t2").val("");//plaintext block becomes empty
});
$("#encrypt2").click(function () {//on clicking encrypt button
    var Plain_text = $("#p_t2").val();// var Plain_text stores the entered plaintext in hexadecimal format
    var key=$("#key2").val();// var key stores the key entered in hexadecimal format
    var Nonce=obj.IV;//Nonce stores the initial value generated by random generator
    var Cipher_text="";//will contain final ciphertext
    for(var i=0;i<Plain_text.length;i+=16){
       Nonce=obj.Encrypt(Nonce,key);//Nonce,key goes for encryption
       var ct_block=obj.xor(Nonce,Plain_text.substr(i,16).padStart(16,'0'));/*it is the output of the encryption function 
       that is fed back to the shift register in OFB*/
       Cipher_text+=ct_block;
    }
    $("#c_t2").val(Cipher_text);//store value of end Ciphertext in c_t2 element
});
$("#decrypt2").click(function () {//on clicking decrypt button
    var Cipher_text = $("#c_t2").val();//ciphertext stores the entered ciphertext
    var key=$("#key2").val();//enter the same key (it will be used in reverse order)
    var Nonce=obj.IV;//Nonce stores the initial value generated by random generator
    var Plain_textt="";//will store the final plaintext
    for(var i=0;i<Cipher_text.length;i+=16){
       Nonce=obj.Encrypt(Nonce,key);//Decrypt the ciphertext 
       var pt_block=obj.xor(Nonce,Cipher_text.substr(i,16).padStart(16,'0'));// Ptext_block xored with Nonce
       Plain_textt+=pt_block;
    }
    $("#p_t2").val(Plain_textt);//store value of end Plaintext in p_t2 element
});
